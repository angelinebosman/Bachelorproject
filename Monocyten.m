...
% Parameters
    ...
    for j= 1:n
        for i=1:n
            ...
            if placement == "inside"
                A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (3*C(i,j) - C(i-1,j) - C(i,j-1) - C(i,j+1)) - l^2/4*rho_m;
                A(k+n,k) = mu_1; %oost roosterpunt
                A(k-n,k) = mu_1; %west roosterpunt
                A(k-1,k) = mu_1; %zuid roosterpunt
            elseif placement == "buitenrand"
                if edge == "west"
                    A(k,k) = -3*mu_1 + (xi_1/C(i,j))*(rho_0/rho_ster)*(2*C(i,j) - C(i-1,j) - C(i,j+1)) - l^2/4*rho_m;
                    A(k+n,k) = mu_1; %oost roosterpunt
                    A(k-1,k) = mu_1; %zuid roosterpunt
                elseif edge == "oost"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j-1) - C(i-1,j))   - l^2/4*rho_m;
                    A(k-n,k) = mu_1; %west roosterpunt
                    A(k-1,k) = mu_1; %zuid roosterpunt
                elseif edge == "noord"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j-1) - C(i,j+1))   - l^2/4*rho_m;
                    A(k-n,k) = mu_1; %west roosterpunt
                    A(k+n,k) = mu_1; %oost roosterpunt
                elseif edge == "zuid"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j+1) - C(i,j-1))- l^2/4*rho_m;
                ...
                elseif edge == "oost_zuid"
                    A(k,k) = -2*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (C(i,j) - C(i,j-1)) - l^2/4*rho_m;
                    A(k-n,k) = mu_1; %west roosterpunt
                    A(k-1,k) = mu_1; %zuid roosterpunt
            ...
            elseif placement == "binnenrand"
                ...
                if edge == "west"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j-1) - C(i-1,j))   - l^2/4*rho_m - sqrt(3)/3*mu_1*d1;
                    A(k-n,k) = mu_1; %west roosterpunt
                    A(k-1,k) = mu_1; %zuid roosterpunt
                elseif edge == "oost"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i-1,j) - C(i,j+1))   - l^2/4*rho_m  - sqrt(3)/3*mu_1*d1;
                    A(k+n,k) = mu_1; %oost roosterpunt
                    A(k-1,k) = mu_1; %zuid roosterpunt
                elseif edge == "noord"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j+1) - C(i,j-1))   - l^2/4*rho_m  - sqrt(3)/3*mu_1*d1;
                    A(k+n,k) = mu_1; %oost roosterpunt
                    A(k-n,k) = mu_1; %west roosterpunt
                elseif edge == "zuid"
                    A(k,k) = -3*mu_1 + xi_1/C(i,j) * rho_0/rho_ster * (2*C(i,j) - C(i,j-1) - C(i,j+1))   - l^2/4*rho_m - sqrt(3)/3*mu_1*d1;
